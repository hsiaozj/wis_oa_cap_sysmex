...
@Path("cap4/formSapRembu")
@Consumes({MediaType.APPLICATION_JSON})
@Produces({MediaType.APPLICATION_JSON})
public class RembuResources extends BaseResource{
	private static Log log = LogFactory.getLog(RembuResources.class);
	
	public static List<CtrlFieldType> loadInfo(){
		List<CtrlFieldType> tmpList = new ArrayList<CtrlFieldType>();
		CtrlFieldType temp = null;
		temp = new CtrlFieldType("AMOUNT", "可用预算金额", Enums.FieldType.VARCHAR, new FormFieldComEnum[] { FormFieldComEnum.TEXT, FormFieldComEnum.TEXTAREA });
		tmpList.add(temp);
