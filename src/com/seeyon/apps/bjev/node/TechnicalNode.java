...
public class TechnicalNode extends BaseSuperNodeAction {
    private static final Log log = LogFactory.getLog(TechnicalNode.class);
    ...
    public SuperNodeResponse executeAction(String token, String activityId, Map<String, Object> param)  {
        ...
        try{
            ...
            log.info("合同审批表（技术标准）流程模板编号"+templeteNumber);
            ...
            if (form4DataMasterBean != null) {
                ...
                log.info("合同审批表（技术标准）处理器业务信息："+businessResult);
            ...
            ex.printStackTrace();
            String message="合同审批表（技术标准）,系统推送数据时出现异常";
            log.info("节点：TechnicalNode:",ex);
