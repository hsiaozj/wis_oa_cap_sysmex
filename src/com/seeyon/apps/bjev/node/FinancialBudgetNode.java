 ...
 * 
 * */
public class FinancialBudgetNode extends BaseSuperNodeAction {
	private static final Log log = LogFactory.getLog(FinancialBudgetNode.class);
	...
	@Override
	public void cancelAction(String arg0, String arg1, Map<String, Object> arg2) {
	}
	...
	public SuperNodeResponse executeAction(String token, String activityId, Map<String, Object> param) {
		...
		try {
			...
			log.info("财务预算审批单流程模板编号"+templeteNumber);
			...
			if (form4DataMasterBean != null) {
				...
				BusinessEnpService financialBudgetEnpService=(BusinessEnpService) AppContext.getBean("financialBudgetEnpService");
				Map<String,Object> financialBudgetData= financialBudgetEnpService.preProcessMethod(documentData);

				// 数据处理器
				Map<String, Object> handleParamMap = new HashMap();
				handleParamMap.put("configCategroy", templeteNumber);
				handleParamMap.put("documentData", financialBudgetData);
				...
				log.info("进入财务处理器以后"+handleResultData);

				handleResultData.put("documentData", financialBudgetData);
				...
				sendDataService = (SendDataService) AppContext.getBean("financialBudgetService");
				Map<String, Object> resultData = sendDataService.doSapSyncMethod(handleResultData);
				log.info("resultData数据结果是："+resultData);

				resultMap = financialBudgetEnpService.finishedProcessMethod(resultData);
			...
			ex.printStackTrace();
			String message="财务预算审批单,系统推送数据时出现异常";
			response.setReturnMsg(message);
			response.setReturnCode(RunAction.BACK.getKey());
			log.info("节点：financialBudgetNode异常:",ex);
