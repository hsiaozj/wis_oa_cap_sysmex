package com.seeyon.apps.bjev.services.impl.purchaseBudget;
 ...
 * 
 * **/
public class PurchaseBudgetServiceImpl extends SendDataServiceImpl implements PurchaseBudgetService{
	private static final Log log = LogFactory.getLog(PurchaseBudgetServiceImpl.class);
	...
	public Map<String, Object> doSapSyncMethod(Map<String, Object> map) throws Exception {
		...
		try {
			...
			//		String templateCode = "PurchaseBudget";
			...
			int index=stringBuilder.indexOf("</ITEMS>");
			stringBuilder.insert(index,subXml);
			...
			for(int i=1;i<4;i++){
				...
				if(StringUtils.isNotBlank(sapNumber)&&StringUtils.isNotBlank(sapNote)) {
					...
					returnResult.put("message", "采购预算单创建成功");
					return returnResult;
				}else if(StringUtils.isBlank(sapNumber)&&StringUtils.isNotBlank(sapNote)) {
					returnResult.put("status", "E");
					returnResult.put("message", "采购预算单创建不成功，请查看SAP返回结果");
					return returnResult;
				}

			}
			returnResult.put("status", "E");
			returnResult.put("message", "采购预算单创建不成功，SAP未及时推送结果数据");

		}catch (Exception ex) {
			ex.printStackTrace();
			log.info("采购预算单创建出现异常"+ex.getMessage());

			String message = "采购预算单创建不成功，系统推送数据时出现异常:";
