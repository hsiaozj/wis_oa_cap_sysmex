package com.seeyon.apps.bjev.services.impl.travelBudget;
...
public class TravelBudgetServiceImpl extends SendDataServiceImpl implements TravelBudgetService{
	private static final Log log = LogFactory.getLog(TravelBudgetServiceImpl.class);
	...
	public Map<String, Object> doSapSyncMethod(Map<String, Object> map) throws Exception {
		...
		try {
			...
			String formsonName = CAP4FormKit.getTableNameByDisPlayName(cap4formBean, "SAP基金中心1", false);//从表表名
			...
			for(int i=1;i<4;i++){
				...
				if(StringUtils.isNotBlank(sapNumber)&&StringUtils.isNotBlank(sapNote)) {
					...
					returnResult.put("message", "国内出差预算单创建成功");
					return returnResult;
				}else if(StringUtils.isBlank(sapNumber)&&StringUtils.isNotBlank(sapNote)) {
					returnResult.put("status", "E");
					returnResult.put("message", "国内出差预算单创建不成功，请查看SAP返回结果");
					return returnResult;
				}

			}
			returnResult.put("status", "E");
			returnResult.put("message", "国内出差预算单创建不成功，SAP未及时推送结果数据");
		}catch (Exception ex){
			ex.printStackTrace();
			log.info("出差预算单创建出现异常"+ex.getMessage());

			String message = "出差预算单创建不成功，系统推送数据时出现异常:";
	...
	private String getSubXml(List<Map> forsonList, FormDataSubBean subBean) throws Exception {
		...
		String FISTLField = CAP4FormKit.getFieldTaleId(subBean, "SAP基金中心1");
		String KOSTLField = CAP4FormKit.getFieldTaleId(subBean, "SAP成本中心1");
		String ARWRBTRField = CAP4FormKit.getFieldTaleId(subBean, "申请金额");
		String POSIDField = CAP4FormKit.getFieldTaleId(subBean, "SAPWBS编码1");
		String EXITEMField = CAP4FormKit.getFieldTaleId(subBean, "SAP费用项目");
		String FIPOSField = CAP4FormKit.getFieldTaleId(subBean, "SAP承诺项目1");
